...
export async function runUsageReport() {
  ...
  const [
    ...
    sbCount('members'),
    // Privacy: no family names — reports transit Anthropic/Resend/Telegram,
    // so families are identified by anonymous label only (see familyActivity).
    sbSelect('families', 'select=id,created_at'),
    sbSelect(
      'completions',
...
EMAIL STRUCTURE:
  ...
  3. 📊 PRODUCT USAGE section:
       Total families, active yesterday (list each active family by its label, e.g. "Family a1b2c3"), total taps yesterday,
...
- <b>📊 Usage</b> section: yesterday's total taps with a trend arrow (↑ ↓ →) vs the 7-day average, active families listed by label, peak tapping hour, any new signups. End this section with a short waitlist line: "<b>Waitlist:</b> {usage.waitlist.total} total ({usage.waitlist.new24h} new in 24h)". If new24h is 0, say "no new in 24h" — do NOT omit.
